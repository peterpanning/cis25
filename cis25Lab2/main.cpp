...
void callMenu(void);
void getDigitOccurence(int, int);
void printOccurrenceArray(int, int);
int getMostOrLeastOddOrEven(int, int);
...
void displayDigitInfoUpdatePeterMutch(){
    ...
    mostOrLeast = getMostOrLeastOddOrEven(1, 0);
    ...
    mostOrLeast = getMostOrLeastOddOrEven(1, 1);
...
int getMostOrLeastOddOrEven (int maxOrMin, int i){
    int mode = 0;
    if (maxOrMin == 1){
        //start at oddOrEven and increment by 2 with >
        do {
        if (tally[i] >= tally[mode]) mode = i;
        i += 2;
        }while(i < 10)
    }
    else{
