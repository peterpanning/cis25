...
BoxPeterMutch::BoxPeterMutch() {
    ...
    cout << "Calling BoxPeterMutch()" << endl;
}
...
void BoxPeterMutch::setBoth(const RectanglePeterMutch& rect, const FractionPeterMutch& frac) {
    base.setBoth(rect.getBottomLeft(), rect.getTopRight());
    height.setBoth(frac.getNum(), frac.getDenom());
}
...
void BoxPeterMutch::print() {
    cout << "\nBottom left is " << base.getBottomLeft() << " and top right is " << base.getTopRight()
    << "with height of " << height << endl;
}
